...
{
protected: //### had to change to protected so goliathobject could access this stuff
   int               owner;
...
//###
class EXPORT_FROM_DLL GoliathObject : public ThrowObject
{
private:
   Vector throwvel;

public:
   CLASS_PROTOTYPE(GoliathObject);
   GoliathObject();
   ~GoliathObject();
   ...
   void Throw(Event * ev);
   void Pickup(Event * ev);
   //void ResetThrowVel(Event * ev);

   virtual void	Archive(Archiver &arc)   override;
   virtual void	Unarchive(Archiver &arc) override;
};

inline EXPORT_FROM_DLL void GoliathObject::Archive(Archiver &arc)
{
   ThrowObject::Archive(arc);

   arc.WriteVector(throwvel);
}

inline EXPORT_FROM_DLL void GoliathObject::Unarchive(Archiver &arc)
{
   ThrowObject::Unarchive(arc);

   arc.ReadVector(&throwvel);
}
//###
