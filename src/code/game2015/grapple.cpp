...
EXPORT_FROM_DLL void Hook::Touch(Event *ev)
{
   ...
   if(other->takedamage)
   {
      ...
      //### fast grapplers can do more damage with their grappling hook
      if(player->HasItem("CTF_Tech_FastGrapple"))
      {
         damg = 20;
         other->Damage(this, owner, damg, worldorigin, velocity, level.impact_trace.plane.normal, 400, 0, MOD_GRAPPLE, -1, -1, 1.0f);
      }
      else
   ...
   if(owner->isClient())
   {
      ...
      //###
      if(player->HasItem("CTF_Tech_FastGrapple"))
         player->SetGrapplePull(worldorigin, CTF_GRAPPLE_PULL_RUNE_SPEED);
      else
...
EXPORT_FROM_DLL void Hook::Setup(Entity *owner, Vector pos, Vector dir)
{
   ...
   if(player->HasItem("CTF_Tech_FastGrapple"))
      velocity = dir * CTF_GRAPPLE_RUNE_SPEED;
   else
